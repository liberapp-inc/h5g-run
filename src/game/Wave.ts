...
class Wave extends GameObject{
    ...
    update() {
        if( Camera2D.x + Util.width >= this.waveX ){
            ...
            Wave.hardRate = Util.clamp( this.waveX / Util.width / 10, 0, 1 );
            
            this.count--;
            if( this.count <= 0 ){
                this.count = randI(4, 10+1) * Util.lerp( 1, 0.5, Wave.hardRate);
                this.type = randI(0, Util.lerp( 1+1, 4+1, Wave.hardRate ) );
            }
    ...
    newHoles(){
        ...
        const px1 = px0 + Util.w(LAND_M_PW) * randF(0.25, 0.8);
    ...
    newSlopeHoles(){
        const px0 = this.waveX;
        const py0 = this.lastPy;
        const px1 = px0 + Util.w(LAND_L_PW) * randF(0.5, 0.8);
        const py1 = Util.h(0.5) + Util.w( randF(0.1, 0.3) );
        new Bar( px0, py0, px1, py1 );
        this.waveX += Util.w(LAND_L_PW);
        this.lastPx = px1;
